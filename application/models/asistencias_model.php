...
class asistencias_model extends CI_Model {
    ...
    public function getRecords() {
        try {
            $query = $this->db->select("A.ID AS ID, A.Usuario AS Usuario, A.Numero AS Numero, A.Fecha AS Fecha, A.Hora AS Hora, A.Estatus AS Estatus, A.Tipo As Tipo", false)->from('sz_asistencias AS A')->get();
            /*
             * FOR DEBUG ONLY
             */
    ...
    public function onComprobarEntrada($Numero, $fecha) {
        try {
            return $this->db->select("A.ID, A.Usuario, A.Numero, A.Fecha, A.Hora, A.Estatus, A.Tipo, A.Empleado, A.EmpleadoT ", false)->from('sz_asistencias AS A')
